...
{block name="override_tpl"}
  ...
  <div class="row">
    <div class="col-lg-7">
      ...
      {if $order->current_state == 2 || $order->current_state == 3 || $order->current_state == 16 || $order->current_state == 23}
        <div id="formMomPanel" class="panel">
          ...
          <div class="table-responsive">
            ...
            <table class="table">
              ...
              <tbody>
                {if sizeof($moms_services)}
                  {assign var="title_service" value=""}
                  {foreach from=$moms_services item=mom}
                    {if $title_service != $mom['nameservice']}
                      {assign var="title_service" value=$mom['nameservice']}
                      <tr class="text-center">
                        <td colspan="6" style="background: #D9F3FD;">{$title_service}</td>
                      </tr>
                    {/if}
                    <tr class="line_mom">
                      <td>{$mom['id_customer']}</td>
                      <td>{$mom['name']}</td>
                      ...
                      <td>
                        {if $mom['selected'] == 0}
                          ...
                          <button id="compareMaps" class="btn btn-default compareMaps" data-origin="{$mom['address']}" data-destination="{$addresses.delivery->address1}, {$addresses.delivery->address2}, {$addresses.delivery->city}">
      ...
      {/if}
      <div id="formQualificationServices" class="panel">
        <div class="panel-heading">
          <i class="icon-check"></i>
          {l s='Qualification' d='Admin.Global'} {l s='Services' d='Admin.Global'}
        </div>
        <div class="table-responsive">
          <input type="hidden" id="order_num" value="{$order->id}" />
          <table class="table">
            ...
            <tbody>
              {if sizeof($qualified_services)}
                {assign var="title_service" value=""}
                {foreach from=$qualified_services item=qualifiedItem}
                  <tr class="line_mom">
                    <td>{$qualifiedItem['id']}</td>
                    <td>{$qualifiedItem['id_order']}</td>
                    <td>{$qualifiedItem['product_name']}</td>
                    <td>{$qualifiedItem['date_add']}</td>
                    <td style="text-align: center;">{$qualifiedItem['qualification']}</td>
                    <td>{$qualifiedItem['description']}</td>
                  </tr>
                {/foreach}
              {/if}
            </tbody>
            ...
            <table class="table" id="orderProducts">
              <thead>
                <tr>
                  ...
                  <th><span class="title_box ">{l s='State' d='Admin.Global'}</span></th>
  ...
  <script type="text/javascript">
    $(document).ready(function()
    {
      ...
      $('.assignMomService').click(function(){
        var r = confirm("Desea asignar la madre #"+$(this).attr("data-idmom")+" a la orden?");
        if (r == true) {
          $.ajax({
            method: "POST",
            url: "/ajax/ApiSetMomOrder.php",
            data : {
              'mom' : $(this).attr("data-idmom"),
              'service' : $(this).attr("data-service"),
              'order' : $("#order_num").val()
            },
          }).done(function(response) {
            if ( !response ) {
              alert("Error. La madre ya no se encuentra disponible para recibir servicios.");
            }
            location.reload();
          });
        }
      ...
      $('.compareMaps').click(function(){
        var address_origin = encodeURIComponent( $(this).attr("data-origin") + " , Colombia" );
        var address_destination = encodeURIComponent( $(this).attr("data-destination") + " , Colombia" );
        var itemLineMom = $(this).parents(".line_mom");
        var buttonCompare = $(this);

        $.ajax({
          method: "POST",
          url: "/ajax/ApiGetDistancePoints.php",
          data : {
          ...
          buttonCompare.attr("disabled", "disabled");
          if ( response.status == "OK" ) {
            itemLineMom.after('<tr class="text-center"><td colspan="6" style="background: #d3f2bf;">Distance: '+response.distance.text+' - Duration: '+response.duration.text+'</td></tr>');
          } else {
            itemLineMom.after('<tr class="text-center"><td colspan="6" style="background: #E08F95;">Error. No results could be found.</td></tr>');
          }
        });
      });
