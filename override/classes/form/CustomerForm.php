...
class CustomerFormCore extends AbstractForm
{
    ...
    public function submit()
    {
        if ($this->validate()) {
            ...
            $product_service = implode(',', $_POST['products_service']);
            $fileone = $_FILES['fileone']['tmp_name'];
            $filetwo = $_FILES['filetwo']['tmp_name'];
            $filetree = $_FILES['filetree']['tmp_name'];
            $filefour = $_FILES['filefour']['tmp_name'];
    ...
    {
        $categorias = Category::getCategories($this->context->language->id);
        $categorias_form = [];

        $products_service = Product::getProducts($this->context->language->id, 0, 99999, 'name', 'ASC', false, true);

        foreach( $categorias[2] as $category){
            $category = $category['infos'];
            array_push($categorias_form,$category);
        }
